...
We evaluate these HSQC-COSY implementations experimentally through the \noah{Sc,S,Cc} and \noah{B,Sc,S,Cc} (\noah*{B} = HMBC, \noah*{Sc} = HSQC-COSY, \noah*{S} = HSQC, and \noah*{Cc} = CLIP-COSY) supersequences.
...
\begin{figure}[!htp]
    ...
    {\phantomsubcaption\label{fig:hsqccosy_comp_hsqc}}%
    ...
    \caption[Comparison of spectra acquired with different HSQC-COSY modules]{
        ...
        \textbf{(\subref*{fig:hsqccosy_comp_hsqc})} Standard HSQC spectrum for reference; only direct peaks are present.
        \textbf{(\subref*{fig:hsqccosy_comp_clip})} HSQC-CLIP-COSY.
        \textbf{(\subref*{fig:hsqccosy_comp_dse})} DSE HSQC-COSY.
        \textbf{(\subref*{fig:hsqccosy_comp_tse_norps})} The first half of the TSE HSQC-COSY; the relay artefacts described in the text are indicated with grey arrows.
        \textbf{(\subref*{fig:hsqccosy_comp_tocsy})} HSQC-TOCSY with \qty{10}{\ms} mixing time.
        \textbf{(\subref*{fig:hsqccosy_comp_tse})} The full TSE HSQC-COSY, with suppression of relay artefacts.
        \andro{}
